  ...
  try {
    if (req.method === 'GET') {
      if (String(req.query?.public || '') === 'reviews') {
        ...
        for (const row of rows || []) {
          const rowReviews = Array.isArray(row.object_data?.reviews)
            ? row.object_data.reviews
            : [];
      ...
      return json(res, 401, {
        error: 'Приватні дані доступні лише у службовому розділі.'
      });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'serviceLogin') {
        if (!SERVICE_PIN || !SERVICE_TOKEN_SECRET) {
          return json(res, 500, {
            error:
              'SERVICE_PIN або SERVICE_TOKEN_SECRET не налаштовано'
          });
        }

        if (!safeEqual(String(body.pin || ''), SERVICE_PIN)) {
          return json(res, 401, {
      ...
      if (action === 'serviceList') {
        if (!verifyServiceToken(req)) {
          return json(res, 401, {
            error:
              'Службова сесія завершена. Увійдіть знову.'
          });
        }

        const rows =
          (await sb(
            'objects?select=*&order=updated_at.desc'
          )) || [];

        return json(res, 200, {
          objects: rows.map(sanitizeServiceRow)
        });
      }

      if (action === 'clientLogin') {
        const row = await validateClient(
          body.objectNumber,
          body.accessKey
        );

        if (!row || !hasPublishedClientView(row)) {
          return json(res, 401, {
            error:
              'Невірний номер об’єкта або ключ доступу.'
          });
        }

        return json(res, 200, {
          object: row.client_view
        });
      }
      ...
      if (action === 'submitReview') {
        ...
        if (text.length < 5) {
          return json(res, 400, {
            error: 'Відгук занадто короткий.'
          });
        }

        const object =
          row.object_data &&
          typeof row.object_data === 'object'
            ? row.object_data
            : {};

        const reviews = Array.isArray(object.reviews)
          ? object.reviews
          : [];
      ...
      if (
        protectedActions.includes(action) &&
        !verifyServiceToken(req)
      ) {
        return json(res, 401, {
          error: 'Немає службового доступу.'
        });
      }
      ...
      if (
        ...
        const object = stripSecretKey(
          current.object_data &&
          typeof current.object_data === 'object'
            ? current.object_data
            : {}
        );
        ...
        if (action === 'updateStatus') {
          ...
          return json(res, 200, {
            object: await patchRow(cloudId, {
              ...
              object_data: next,
              client_view: hasPublishedClientView(current)
                ? publicView(
                    next,
        ...
        if (action === 'clientData') {
          return json(res, 200, {
            ...
            published: hasPublishedClientView(current)
          });
        }

        if (action === 'serviceClientView') {
          if (!hasPublishedClientView(current)) {
            return json(res, 404, {
              error:
                'Особисту картку ще не створено.'
            });
          }

          return json(res, 200, {
            object: current.client_view
          });
        }

        if (action === 'regenerateAccessKey') {
          const key = makeAccessKey();
          const next = stripSecretKey(object);

          const row = await patchRow(cloudId, {
            access_key_hash: hashAccessKey(key),
            object_data: next,
            client_view: publicView(
              next,
              current.object_number,
              next.cloudStatus ||
                current.status ||
          ...
          const row = await patchRow(cloudId, {
            ...
            client_view: hasPublishedClientView(current)
              ? publicView(
                  next,
                  current.object_number,
                  next.cloudStatus || current.status
                )
              : {}
          });
      ...
      const mode = action || 'publishClient';
      const status = String(
        body.status ||
        object.cloudStatus ||
        'new'
      ...
      if (mode === 'historySave') {
        if (cloudId) {
          ...
          const objectNumber =
            current.object_number ||
            await nextObjectNumber();

          const updated = {
            ...stripSecretKey(object),
            objectNumber
          };
          ...
          return json(res, 200, {
            created: false,
            published: hasPublishedClientView(row),
            object: row
          });
        }
        ...
        const inserted =
          await insertWithGeneratedObjectNumber(
            objectNumber => {
              const updated = {
                ...stripSecretKey(object),
                objectNumber
      ...
      if (mode === 'publishClient') {
        if (cloudId) {
          ...
          const firstPublication =
            !hasPublishedClientView(current);

          const key = firstPublication
            ? makeAccessKey()
            : '';
          ...
          const updated = {
            ...
            objectNumber,
            reviews:
              current.object_data?.reviews ||
              object.reviews ||
              [],
            serviceData:
              current.object_data?.serviceData ||
              object.serviceData ||
              {}
          };
        ...
        return json(res, 201, {
          ...
          published: true,
          accessKey: key,
          object: sanitizeServiceRow(inserted.row)
        });
      }
    ...
    if (isUniqueViolation(error)) {
      return json(res, 409, {
        error:
          'Номер об’єкта вже зайнятий. Спробуйте зберегти ще раз.'
      });
    }
